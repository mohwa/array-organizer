# Various ways which using together with the native API
...
toArray({ x: 'spray', y: 'limit', z: 'elite', xx: 'exuberant', yy: 'destruction', zz: 'present' }).filter(({ v, k }) => v.length > 6); // [{ k: "xx", v: "exuberant" }, { k: "yy", v: "destruction" }, { k: "zz", v: "present" }]
...
// Will be filled 7 from an 3 index to an end index in the array object
...
// Will be copied second element to first element

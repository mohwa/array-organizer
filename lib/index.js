...
// toArrayAll 도 만들어야하지않을까? 원시값에 대응하는 이런것([1])
 ...
 */
export function indexOf(v, searchValue, fromIndex) {
 ...
 */
export function lastIndexOf(v, searchValue, fromIndex) {
  const _v = toArray(v, vv => vv);

  if (!isUndefined(fromIndex)) {
    return _v.lastIndexOf(searchValue, fromIndex);
  }
  return _v.lastIndexOf(searchValue);
}

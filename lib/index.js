...
import { splice, _toArray, ascOperator, descOperator, bindToFunction } from './_utils';

const { isObject, isUndefined } = type;
/**
...
export function copyWithin(v, t, start, end = v.length) {
  return _copyWithin(_toArray(v), t, start, end + 1);
}
...
export function fill(v, vv, start, end = v.length) {
  return _fill(_toArray(v), vv, start, end + 1);
}
...
export function findIndex(v, ...args) {
  return _findIndex(_toArray(v), ...args);
}
...
export function flatMap(v, ...args) {
  return _flatMap(_toArray(v), ...args);
}
...
export function asc(v) {
  return _toArray(v, vv => vv).sort((v1, v2) => {
...
export function reduce(v, f, initV) {
  ...
  let acc;
  _toArray(v, (vv, k) => {
    acc = !isUndefined(initV) ? initV : vv;
    acc = _f(acc, vv, k, v);
  });
  return acc;
}
